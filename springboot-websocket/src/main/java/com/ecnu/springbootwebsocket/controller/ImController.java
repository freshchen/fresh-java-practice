...
import com.ecnu.springbootwebsocket.entity.ChatMessage;
...
public class ImController {
    ...
    public JSONObject getUserlist() {
        JSONObject users = (JSONObject) webSocketConfig.usersMap;
        return users;
    }
    ...
    public ChatMessage greeting(ChatMessage message) throws Exception {
        ...
        String content = date + "【" + message.getName() + "】say：" + message.getContent();
    ...
    public void privatechat(ChatMessage message) throws Exception {
        ...
        String date = simpleDateFormat.format(new Date());
        String content = date + "【" + userid + "】say to you：" + ctx;
        String contents = date + " you say to【" + touser + "】：" + ctx;

        template.convertAndSendToUser(userid, "/topic/private", new ChatMessage(touser, contents, touser, date));
        Thread.sleep(1000);
        if ("rebot".equals(touser)) {
            touser = userid;
            ...
            content = date + "【rebot】say to you ：" + text;
            template.convertAndSendToUser(touser, "/topic/private", new ChatMessage("rebot", content, "rebot", date));
            return;
        }
